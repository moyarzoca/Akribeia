...
BuildEvalNetwork[Nx_, pathJSON_, conf_: <||>] := Module[
	{
		...
		referenceFields, parameters, C0, betaValue
	},
	WeightBias = Import[pathJSON, "RawJSON"];

	alphaValue = RequireInputObjectValue[conf, "alpha"];
	betaValue = RequireInputObjectValue[conf, "beta"];
	...
	evaluateField[fieldName_, grid_] := SetPrecision[(models[fieldName][#, betaValue] &) /@ grid, Pre];
	...
	C0 = SetPrecision[models["C0"][betaValue], Pre];
...
LoadInitialData[InputObject_] := Module[
	...
	Print[
		...
		"Parameters: \[Beta] = " <> ToString[\[Beta] 1.] <>
		"  rh = " <> ToString[rh 1.] <>
		"   C0 = " <> ToString[C0 1.] <>
		"   Oh = " <> ToString[Oh*1.]
	];
